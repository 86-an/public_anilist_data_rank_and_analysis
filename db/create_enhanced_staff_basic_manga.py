...
def create_staff_role_table(cursor):
    """スタッフロールテーブルを作成"""
    cursor.execute('''DROP TABLE IF EXISTS staff_role''')
    cursor.execute('''
...
def populate_staff_role_table(cursor):
    """スタッフロールテーブルにデータを投入"""
    print("スタッフロールデータを投入中...")
    cursor.execute('''
...
def main():
    ...
    print("="*70)
    print("拡張スタッフ基本統計テーブル作成ツール (manga_data.db)")
    print("="*70)
    ...
    # スタッフロールテーブル作成
    print("\n=== スタッフロールテーブル作成 ===")
    print("staff_roleテーブル作成中...")
    create_staff_role_table(cursor)
    populate_staff_role_table(cursor)
